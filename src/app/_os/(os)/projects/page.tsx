...
// Helper: colores distintivos para labels según categoría
const labelColorMap: Record<string, string> = {
  Arquitectura: "bg-blue-500/10 text-blue-400 border-blue-500/20",
  ...
  Producto: "bg-primary/10 text-primary border-primary/20",
};

function getLabelColor(label: string) {
  return labelColorMap[label] || "bg-secondary border border-border text-muted-foreground";
}
  ...
  return (
    <div className={`group bg-card border border-border rounded-xl p-3.5 cursor-pointer shadow-sm transition-all hover:border-primary/40 hover:shadow-[0_4px_20px_rgba(0,255,135,0.03)] flex flex-col gap-3 relative overflow-hidden ${isDone ? "opacity-60" : ""}`}>
      ...
      <div className="flex justify-between items-center text-xs text-muted-foreground font-mono">
        <span>{id}</span>
        {entityLink && (
          <div className="flex items-center gap-1 bg-primary/10 px-1.5 py-0.5 rounded text-[10px] uppercase font-sans font-medium text-primary border border-primary/20 group-hover:border-primary/50 transition-colors">
      ...
      {labels.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {labels.map((label: string) => (
            <span key={label} className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${getLabelColor(label)}`}>
              {label}
            </span>
          ))}
        </div>
