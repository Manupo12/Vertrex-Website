        ...
        <div className="rounded-2xl border border-border bg-background/40 p-6">
          ...
          <div className="space-y-4">
            ...
            <ConversationBubble
              role="assistant"
              title="IA COO"
              body={`Snapshot actual: ${data.summary.projects} proyectos visibles, ${data.summary.openTasks} tareas abiertas, ${data.summary.pendingTickets} tickets activos y ${data.openClaw.activeSessions} sesiones OpenClaw en línea.`}
            />
            <div className="rounded-xl border border-border bg-secondary/30 p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">getProjects · getTickets · getFinanceSummary · searchMemory · callOpenClaw</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <Panel title="Proyectos recientes">
            <div className="space-y-3">
              {data.recentProjects.map((project) => (
                <div key={project.id} className="rounded-xl border border-border bg-background/40 p-3">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-foreground">{project.name}</p>
                    <span className="text-xs font-mono text-primary">{project.progress}%</span>
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">Estado: {project.status}</p>
                </div>
              ))}
            </div>
          </Panel>
...
function getTabDescription(tab: AITab, data: AIControlCenterData) {
  switch (tab) {
    case "autonomous":
      return `${data.autonomousRuns.length} ejecuciones recientes disponibles para revisión.`;
    case "memory":
    ...
    default:
      return `Contexto activo: ${data.summary.projects} proyectos, ${data.summary.openTasks} tareas abiertas.`;
  }
}
