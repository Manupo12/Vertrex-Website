...
export default function LoginScreen({ defaultRole = "team" }: LoginScreenProps) {
      ...
      <div className="relative hidden w-[45%] flex-col justify-between border-r border-border bg-[#050505] p-12 lg:flex">
        ...
        <div className="relative z-10 w-full max-w-xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            <StatusCard icon={FolderKanban} label="Proyectos Activos" value="24" detail="Operaciones en curso" color="text-primary" />
            <StatusCard icon={Users} label="Usuarios Activos" value="14.2k" detail="En SaaS, Apps y Webs" color="text-blue-400" trend="+12%" />
            <StatusCard icon={Heart} label="Comunidad" value="15" detail="Proyectos open source" color="text-purple-400" />
            <StatusCard icon={Rocket} label="Roadmap" value="06" detail="Lanzamientos planeados" color="text-amber-400" />
          </div>
        </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative flex items-center overflow-hidden rounded-xl border border-border bg-card transition-all focus-within:border-primary">
                <input
                  ...
                  placeholder={role === "team" ? "admin" : "budaphone"}
                  className="w-full bg-transparent px-4 py-3 text-sm text-white outline-none placeholder:text-muted-foreground/30"
                  autoComplete="username"
                  required
                />
                {role === "team" ? (
                  <div className="px-4 py-3 text-xs font-bold text-muted-foreground/40">@vertrex.co</div>
                ) : null}
              </div>
              <p className="text-[11px] text-muted-foreground">
                {role === "team"
                  ? "Usa tu identificador interno o correo Vertrex."
...
function StatusCard({
  ...
  return (
    <div className="group rounded-2xl border border-border bg-card/40 p-5 backdrop-blur-sm transition-all hover:border-white/20">
      <div className="mb-4 flex items-center justify-between">
        <div className={`rounded-lg border border-border bg-background p-2 ${color}`}>
